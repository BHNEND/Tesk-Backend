...
const ALLOWED_RESOLUTIONS = new Set(["512", "1k", "2k", "4k"]);
const MAX_IMAGES = 14;

export const yunwubanana2Handler: TaskHandler = {
  preview(input: any, identifier?: string, upstreamIdentifier?: string): HandlerPreview {
    const id = identifier || "yunwubanana2";
    ...
    return {
      ...
      body,
      meta: { identifier: id, handler: "yunwubanana2", platform: "yunwu-gemini" },
    };
  },
  ...
  async execute(ctx: TaskHandlerContext): Promise<any> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`yunwubanana2 request failed: ${response.status} - ${errorText}`);
    }
