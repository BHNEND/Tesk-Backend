...
export const yunwubananaproHandler: TaskHandler = {
  preview(input: any, identifier?: string, upstreamIdentifier?: string): HandlerPreview {
    const id = identifier || "yunwubananapro";
    ...
    return {
      ...
      body,
      meta: { identifier: id, handler: "yunwubananapro", platform: "yunwu-gemini" },
    };
  },
  ...
  async execute(ctx: TaskHandlerContext): Promise<any> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`yunwubananapro request failed: ${response.status} - ${errorText}`);
    }
